...
## 🔐 科內維護模式說明
- **預設密碼**：`119` (亦相容 `admin119` 或 `minli119`)——這組僅為尚未設定自訂密碼時的備援值，
  **且因為早期版本曾將此密碼直接寫在公開原始碼中，強烈建議盡快改用下方方式設定專屬密碼**。
- **設定專屬管理密碼（建議）**：
  - 前往 Streamlit Cloud → 你的 App → **Settings → Secrets**，貼上：
    ```toml
    admin_password = "請改成你自己的高強度密碼"
    ```
  - 本機開發則在專案根目錄建立 `.streamlit/secrets.toml`（此檔已列在 `.gitignore`，不會被上傳）：
    ```toml
    admin_password = "請改成你自己的高強度密碼"
    ```
  - 設定後，側邊欄會自動改用這組新密碼登入，且不會再顯示「尚未設定」的提醒。
- 登入後可於側邊欄或分頁使用：
  1. **Excel 式線上即時編輯**
  2. **新增業務項目**
  3. **單筆詳細維護與刪除**
